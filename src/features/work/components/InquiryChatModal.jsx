...
const InquiryChatModal = ({ inquiry, onClose, onBookingUpdated, onError }) => {
  ...
  const styles = {
    ...
    modal: { background: 'var(--gl-surface)', color: 'var(--gl-text)', borderRadius: '16px 16px 0 0', display: 'flex', flexDirection: 'column', maxWidth: '520px', width: '100%', height: '80vh', maxHeight: '80vh', boxShadow: '0 -18px 42px rgba(15, 23, 42, 0.22)' },
    header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderBottom: '1px solid var(--gl-border)', flexShrink: 0 },
    title: { fontSize: '15px', fontWeight: 700, color: 'var(--gl-text)', margin: 0 },
    subtitle: { fontSize: '12px', color: 'var(--gl-text-3)', margin: '2px 0 0' },
    closeButton: { background: 'none', border: 'none', fontSize: '24px', color: 'var(--gl-text-3)', cursor: 'pointer', padding: 0, width: '32px', height: '32px' },
    messages: { flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px', background: 'var(--gl-surface-2)' },
    empty: { color: 'var(--gl-text-3)', textAlign: 'center', padding: '24px 10px' },
    ...
    clientBubble: { background: 'var(--gl-surface)', color: 'var(--gl-text)', border: '1px solid var(--gl-border)' },
    workerBubble: { background: 'var(--gl-blue)', color: '#ffffff' },
    ...
    actions: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', padding: '12px', background: 'var(--gl-surface)', borderTop: '1px solid var(--gl-border)', flexShrink: 0 },
    actionButton: { padding: '10px 12px', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 700, cursor: 'pointer' },
    secondaryButton: { background: 'var(--gl-surface-2)', color: 'var(--gl-text)' },
    primaryButton: { background: 'var(--gl-blue)', color: '#ffffff' },
    inputArea: { display: 'flex', gap: '8px', padding: '12px', background: 'var(--gl-surface)', borderTop: '1px solid var(--gl-border)', flexShrink: 0 },
    input: { flex: 1, padding: '10px 12px', border: '1px solid var(--gl-border-strong)', borderRadius: '8px', fontSize: '14px', fontFamily: 'inherit', outline: 'none', background: 'var(--gl-surface)', color: 'var(--gl-text)' },
    sendButton: { padding: '10px 16px', background: replyText.trim() && !isSaving ? 'var(--gl-blue)' : '#94a3b8', color: '#ffffff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 700, cursor: replyText.trim() && !isSaving ? 'pointer' : 'not-allowed' },
    ...
    quoteModal: { background: 'var(--gl-surface)', color: 'var(--gl-text)', borderRadius: '12px', width: 'min(92vw, 520px)', padding: '18px', display: 'grid', gap: '12px', boxShadow: '0 18px 50px rgba(15, 23, 42, 0.28)' },
    label: { display: 'grid', gap: '6px', fontSize: '13px', fontWeight: 700, color: 'var(--gl-text-2)' },
    quoteInput: { minHeight: '44px', padding: '10px 12px', border: '1px solid var(--gl-border-strong)', borderRadius: '8px', fontSize: '14px', fontFamily: 'inherit', background: 'var(--gl-surface)', color: 'var(--gl-text)' },
    quoteTextarea: { minHeight: '100px', padding: '10px 12px', border: '1px solid var(--gl-border-strong)', borderRadius: '8px', fontSize: '14px', fontFamily: 'inherit', resize: 'vertical', background: 'var(--gl-surface)', color: 'var(--gl-text)' },
